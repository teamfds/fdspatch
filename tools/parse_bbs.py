...
bbs_uexp_files = [y for x in os.walk(f"{mod_root}/res") for y in glob(os.path.join(x[0], f"{sys.argv[1]}.uexp"))]

for file in bbs_uexp_files:
    unpack_dest = file.replace("res", "build/parsed_bbs").replace(".uexp", ".bbscript")
    parse_dest = unpack_dest.replace(".bbscript", ".txt")

    os.makedirs(os.path.dirname(unpack_dest), exist_ok=True)

    tools.bbspack(["extract", file, unpack_dest])
